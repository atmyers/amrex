...
void checkAnswer (const amrex::SparseBins<int>& bins)
{
    BL_PROFILE("checkAnswer");
    const auto perm = bins.permutationPtr();
    const auto bins_ptr = bins.getNonZeroBinsPtr();
...
void test (int nbins, const amrex::Vector<int>& items)
{
    amrex::SparseBins<int> bins;
